class SearchController < ApplicationController
  ...
  def index
    ...
    @reference_results = PgSearch.multisearch(params[:query]).includes(:searchable).where(searchable_type: 'Reference').map(&:searchable)
    @reference_results = Kaminari.paginate_array(@reference_results).page(params[:resource]).per(4)

    @tag_results = PgSearch.multisearch(params[:query]).includes(:searchable).where(searchable_type: 'Tag').map(&:searchable)
